...
const AdminNewOrderToast = (ordersOrCount) => {
  const sourceTitles = {
    bot: "бота",
    website: "сайта",
    mobile: "мобильного приложения",
  };

  const orders = Array.isArray(ordersOrCount) ? ordersOrCount : [];
  const totalCount = Array.isArray(ordersOrCount) ? orders.length : ordersOrCount;
  ...
  const sourceParts = Object.entries(sourceCounts).map(
    ([key, count]) => `${count} от ${sourceTitles[key]}`
  );
  const sourceMessage =
    sourceParts.length > 0
      ? `Новые заказы: ${sourceParts.join(", ")}.`
      ...
      <main className="w-11/12 mx-auto flex gap-5 flex-col">
        <div className="flex justify-between items-center w-full">
          <div className="space-y-2">
            ...
            <p className="text-thin text-sm">
              Количество новых заказов {totalCount}
            </p>
          </div>
          <button>
            <IoMdNotificationsOutline className="text-primary text-5xl" />
